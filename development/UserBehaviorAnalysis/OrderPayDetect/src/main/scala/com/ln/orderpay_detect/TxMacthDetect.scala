package com.ln.orderpay_detect
...
object TxMacthDetect {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 读取支付到账事件流
    val resource_ReceiptLog = getClass().getResource("/ReceiptLog.csv")
    val receiptEventStream = env.readTextFile(resource_ReceiptLog.getPath)
      .map(data => {
        val dataArray = data.split(",")
